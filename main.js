...
function reloadall() {
  document.getElementById("click").innerHTML =
    "Clicks: " + addcomma(moneyup) + " | Clicks per Sec: " + addcomma(msec);
  document.getElementById("total").innerHTML = "Clicks: " + addcomma(money);
  document.getElementById("cat").innerHTML =
    catown + "-Clicker Cats: " + addcomma(catcost) + " | +" + addcomma(catadd) + "/sec";
  document.getElementById("worker").innerHTML =
    workerown + "-Workers: " + addcomma(workercost) + " | +" + addcomma(workadd) + "/sec";
...
function upgrade(name) {
  if (name == "clicker cat") {
    if (money >= catcost && catown < 50) {
      ...
      catcost = catcost * 2;
      document.getElementById("cat").innerHTML =
        catown + "-Clicker Cats: " + addcomma(catcost) + " | +" + addcomma(catadd * cboost) + "/sec";
    } else if (catown == 50) {
      document.getElementById("cat").innerHTML =
        catown + "-Clicker Cats: MAX | +15% clicks/sec";
    }
  }
  ...
  if (name == "worker") {
    if (money >= workercost && workerown < 50) {
      ...
      document.getElementById("worker").innerHTML = 
        workerown + "-Workers: " + addcomma(workercost) + " | +" + addcomma(workadd * wboost) + "/sec";
    } else if (workerown == 50) {
      document.getElementById("worker").innerHTML =
        workerown + "-Workers: MAX | +35% click/sec";
    }
  }
  ...
  document.getElementById("click").innerHTML =
    "Clicks: " + addcomma(moneyup) + " | Sec: " + addcomma(msec);
  document.getElementById("total").innerHTML = "Clicks: " + addcomma(money);
}